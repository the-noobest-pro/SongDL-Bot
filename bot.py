...
async def yt_dl_audio(client, cb):
    ...
    try:
        ...
        if f"{rip_data['id']}.mp3.jpg" in dir:
            ...
            thumb = None
        ohhkay = await client.send_audio(-1001574914096, rip_file, duration=int(rip_data["duration"]))
        await asyncio.sleep(1)
        await cb.edit_message_media(InputMediaAudio(f"{ohhkay.audio.file_id}", title=str(rip_data["title"]), duration=int(rip_data["duration"]), thumb=thumb))
    except Exception as e:
        print (e)
